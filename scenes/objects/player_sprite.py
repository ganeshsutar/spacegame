...
class PlayerSprite(pygame.sprite.Sprite):
    ...
    def updateImage(self):
        ...
        if self.showShield and self.shield != -1:
            self.blitAtCenter(shields[self.shield])

        # pygame.draw.rect(self.image, (255,255,255,150), self.image.get_rect(), 1)
        # pygame.draw.circle(self.image, (255,255,255,150), self.image.get_rect().center, self.image.get_rect().width/2, 1)
    ...
    def blinkShield(self):
        self.showShield = True
        self.updateImage()
        threading.Timer(1, self.offShield).start()
